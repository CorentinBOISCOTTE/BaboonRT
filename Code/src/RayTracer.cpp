...
RayTracer::RayTracer(const int samplesPerPixel) : m_samplesPerPixel(samplesPerPixel)
{
                       ...
                       HittableList world)
{
	...
	for (uint16_t y = 0; y < height; ++y)
	{
		for (uint16_t x = 0; x < width; ++x)
		{
			...
			const float scale = 1.0f / (static_cast<float>(m_samplesPerPixel) / static_cast<float>(m_samplesPerPixel));
			r *= scale;
			g *= scale;
			b *= scale;
	...
	//	{
	//		threadPool->Enqueue([ty, height, tx, width, pixel00Loc, pixelWidth, pixelHeight, cameraCenter, &framebuffer, world]()
	//			{
	...
	//
	//						framebuffer[y * width + x] = ray.RayColor(world);
	//					}
	//				}
	//			});
	//	}
